...
export class IngredientService {
    ...
    public Nutrients: Promise<INutrientInfo[]> | INutrientInfo[];
    ...
    public async LoadNutrientTypes(forceReload: boolean = false): Promise<INutrientInfo[]> {
        if (!forceReload && this.Nutrients) return this.Nutrients;
        this.Nutrients = new Promise<INutrientInfo[]>(async resolve => {

            let response = await this.http.get(this.serverService.GetBase() + "/nutrients");

            response.subscribe(data => {
                resolve(data as INutrientInfo[]);
            }, error => {
                resolve([]);
            });
        });
    ...
    public async GetNutrientData(code: number): Promise<INutrientInfo> {
        if (!this.Nutrients) return null;
        return (await this.Nutrients).find(n => n.code == code);
    }
    ...
    public async DeleteCustomUnit(currentType: IngredientType, currentUnit: Unit, descendentId?: string): Promise<void> {
        let body: DeleteCustomUnitRequest = {
            ingredientTypeId: currentType.guid,
            unitToDeleteId: currentUnit.id,
            descendent: descendentId
        };
