...
export function RegisterRoutes(app: express.Express) {
    ...
    app.get('/nutrients/',
        function(request: any, response: any, next: any) {
            authenticationReqMiddleware(defaultAuthentication, request, response, false, ['edit-ingredients'], (error) => {
            }).then((user) => {
                const args = {};
                const controller = new NutrientController();
                const promise = controller.GetAllNutrients(request.body as void, user,);
                ProcessPromiseResponse(controller, promise, response, next, (error) => {
                });
            }).catch((error) => {
                console.error(error);
